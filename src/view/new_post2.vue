<template>
  ...
  const uploadArticleBlockchain = async (data) => {
    ...
    console.log("uploadArticleBlockchain",  data.title, data.content, data.publicationDate);
    ...
    const transactionParameters = {
      ...
      to: contractAddress,
      value: "0",
      data: socialContract.methods.publish(data.title, data.content, publicationDate).encodeABI(),
    };
    try {
